...
1. **Free-space is anchor-derived via append-only *clean anchors*, with no delta arithmetic over mutable
   tables.** A *clean anchor* is a reconciled free-space observation taken when no write is in flight,
   carrying `anchor_free_bytes`, the `identity+capacity epoch`, `anchor_at`, and mount-identity proof.
   Between anchors a drive is either **dirty** (a write is/was in flight) or **offline** (no writes
   possible). Therefore: **offline free = the latest clean anchor's `anchor_free_bytes`** (nothing can
   have changed since); mounted-and-clean free = live `df`; **dirty ⇒ `unknown`**. A release or
   allocation is reflected by the *next clean anchor after reconciliation* — never by crediting a
   vanished catalog row or differencing a scalar watermark over mutable/tombstoned rows. Values clamp to
   `[0, usable_capacity_for_epoch]`. Empty registration is the trivial first clean anchor; a full mounted
   inventory/reconciliation appends a fresh clean anchor for an already-populated drive — the supported
   recovery from `unknown`. Anchors are immutable per `identity+capacity epoch`; a resize is an
   explicit, audited epoch transition that appends a new anchor.
...
3. **Dirty protocol (generation-based, single-writer, crash-safe).** Dirtied **durably before any
   ...
   registration/annex-init, recovery. NULL `stored_bytes` is missing evidence, never a proven zero.
...
7. **Placement optimization is separate from safety, and is one concrete deterministic rule.** After a
   ...
   **lexicographically** over the ordered objectives — it is *not* required to prove optimality (that
   ...
   `tiered_v2` versions this exact rule; quality beyond feasibility is advisory.
8. **Serialized control is catalog-backed with a real lease.** A monotonic `graph_revision` plus a
   durable lease defining owner/session id, acquire/renew, heartbeat/expiry, crash recovery, safe
   operator takeover, a **monotonically increasing fencing token validated on every worker catalog
   write**. The commit protocol does **not** hold `BEGIN IMMEDIATE` across the solve: acquire lease →
...
- Per identity, append `clean_anchor` rows (`anchor_free_bytes`, `identity_capacity_epoch`, `anchor_at`,
  `mount_identity`); plus `observed_free_bytes`/`observed_at` (diagnostic), `free_evidence` enum, and a
  generation-based `dirty` marker. No writable current-free field.
- Offline free = latest clean anchor; mounted-clean = live `df` with mount-identity proof; dirty ⇒
  `unknown`. No allocation/release ledger and no watermark-over-mutable-tables differencing.
- **Recovery from `unknown`** = a mounted inventory/reconciliation (prove identity → inventory staging/
  orphans → reconcile catalog↔annex → append a clean anchor). Populated drives fully supported.
...
- 36a exposes, for each required copy, **all** partial candidates with deterministic reuse costs
  (reusable present files, missing-work identity, re-download vs relocation cost). It makes **no**
...
- **Never delete verified rows** — a non-chosen partial's files are preserved as policy-drift until an
  explicit #37 op. Annex-to-annex relocation is **not required this phase**; 36a merely *prices* it.
...
- Ordered objectives: (1) tier/failure-domain; (2) immovable work; (3) feasible whole-model arrangement
  (the safety verdict); (4) minimize relocation/re-download (consumes 36a costs); (5) preserve large
...
- Search bound is a deterministic state-count over a canonical traversal; wall-clock is only an
  emergency cap. Correction from review: canonical path already has the label tiebreak (capacity.py:753);
...
- Eligibility + lifecycle (not `plan_drives` membership, and not `presence`) gate placement **and**
  bootstrap — `plan.bootstrap()` (plan.py:160) re-adds every registered drive on startup. **Complements**
  (not subsumes) DEF-029.
...
- **#35** — offline=latest-clean-anchor, dirty⇒unknown, mount-identity failures (wrong volume, missing
  NAS mount, replaced fs, stale path) ⇒ `unknown`, anchor recovery on a populated drive, capacity-epoch
  resize, **registration crashes** (before row creation, during clone, during annex-init, before first
  anchor), NULL vs proven-zero `stored_bytes`, unprovable-provenance⇒`unknown`, every consumer.
- **#36a/#38** — alternatives emitted without pinning; global feasibility (a partial fitting itself but
  breaking the fleet is not chosen); graded Gate-B outcomes; feasibility-existence vs optimization
  separation; deterministic large-block metric asserted; adversarial packing; 10k-candidate performance.
...
- **#39** — lease acquire/renew/expiry/**crash recovery/operator takeover/fencing-token validation**,
  no `BEGIN IMMEDIATE` held across the solve, cross-writer atomicity (portal + CLI writers), config-in-
  snapshot / no file reread mid-lease, execution-authority-task equivalence, live-lease predicate, all
