...
| Recovery/identity repair/clean publisher | `require_publication_clear` on serial repair, reconcile, owned recovery, clean-anchor, preview/approve, restore/Slice source admission, hash repair, and expired-session recovery. Map-only PREPARED registration blocks clean-anchor. `_CLOSING` remains the only clean-anchor exemption | Stage 2 conversion/cutover. Frozen batch children, Stage 1 crash resume, and leftover registration close are DEC-162 |
