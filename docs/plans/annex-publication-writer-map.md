...
| Recovery/identity repair/clean publisher | `require_publication_clear` on serial repair, reconcile, owned recovery, clean-anchor, preview/approve, restore/Slice source admission, hash repair, and expired-session recovery. Map-only PREPARED registration blocks clean-anchor. `_CLOSING` remains the only clean-anchor exemption | Failed/skipped Fill vs frozen batch children; fault-injection; leftover CATALOG_PUBLISHED registration resume |
