...
Hugging Face has 1M+ repos. Most are noise. ModelArk catalogs metadata broadly
(cheap — a few MB), then downloads only curated full-precision weights, tracked by
**git-annex** across a fleet of external HDDs + a NAS that mostly live offline. The
git repo is the *map* of the library; the bytes never touch git.
...
- **Catalog** — DuckDB (`catalog/catalog.duckdb`, rebuildable). Source of truth for
...
4. **Fill** — per shard: HF download → sha256 vs HF's canonical hash → compress → **canary**
   → drop the original → git-annex add → record. Throttled (default 2 TB/24 h), resumable at
...
compression:            # DEC-022 codec gate
  ...
  threads: 4                 # ZipNN internal threads (shared-memory: no extra peak RSS, no pickling)
```
...
The portal runs as *your* user (typically a `systemd --user` service) so the catalog and the
...
10 GB shards); the codec gate; must-have 2-copy replication; crash-resume; and the portal's
four views including the live Fill run surface. Restorability is production-verified (`DIS-002`).
...
- **Library Audit** — a fleet-wide integrity check (sample + re-canary) isn't built yet (#23).
- **Download Status view** — the portal doesn't yet surface `fetch_events` history (#18).
...
- **Self-describing drives** — a re-plugged shelf drive isn't yet fully self-documenting (#14).
