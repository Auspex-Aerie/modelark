...
def _with_revision(mutate) -> dict:
    """Run a selection write under planner_revision bump (PR-08 A3)."""
    from modelark.proposal import GraphResult, graph_write

    def op(c):
        # selection_api uses data.conn(); graph_write owns the transaction on that connection.
        value = mutate()
...
def summary() -> dict:
    try:
        ...
        # Catalog paths without a portal ui_cache (CLI/tests) still need selection writers.
        by, recent = [], {}
...
def toggle(repo_id: str, on: bool) -> dict:
    if on:                                          # addition: never Fill-guarded; still bumps revision
        def add():
            ...
            return summary()
        return _with_revision(add)

    def mutate():                                   # deselect: guarded while Fill is live
        def body():
            data.conn().execute("DELETE FROM selection WHERE repo_id=?", [repo_id])
            return summary()
...
def clear() -> dict:
    def mutate():
        def body():
            data.conn().execute("DELETE FROM selection")
            return summary()
        return _with_revision(body)
    return _guarded(mutate)
